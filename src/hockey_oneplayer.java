...
public class hockey_oneplayer implements Initializable , EventHandler<KeyEvent>{
	@FXML
	...
	public void run(GraphicsContext gc) {
		...
		if(gameStarted) {
			...
			//電腦跟著球移動
			if(j == 0 || j == 3 || j == 4) {
				if(ballXPos < width - width  / 2) {
					playerTwoYPos = ballYPos - PLAYER_HEIGHT / 2;
				}else {
					playerTwoYPos = ballYPos + PLAYER_HEIGHT / 2;
				}
			}else if(j == 2 || j == 1 ) {
				if(ballXPos < width - width  / 2 && ballYPos > 200) {
					playerTwoYPos = ballYPos - 65;
				}else if (ballXPos < width - width  / 2 && ballYPos < 200){
					playerTwoYPos = ballYPos + 35;
				}
			}
			
			//電腦只在球門間移動
			if(playerTwoYPos - 25 < 100) {
				playerTwoYPos = 100;
		    }else if(playerTwoYPos + 25 > 250) {
		    	playerTwoYPos = 250;
		    }
		...
		if(ballXPos > width - 28 && ballYPos > 292 || ballXPos > width - 28  && ballYPos < 108) {
			ballXSpeed *= -1;
			
		}
		if(ballXPos < 18 && ballYPos > 292 || ballXPos < 18  && ballYPos < 108) {
			ballXSpeed *= -1;
		...
		if(ballXPos < playerOneXPos - PLAYER_WIDTH) {
			...
			gameStarted = false;
			if(scoreP2 == 5) {
				Main.currentStage.setScene(Main.Loseend);
			}
		...
		if(ballXPos > playerTwoXPos + PLAYER_WIDTH) {  
			...
			gameStarted = false;
			if(scoreP1 == 5) {
				Main.currentStage.setScene(Main.winend);
			}
		}
